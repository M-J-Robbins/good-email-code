...
This can be combined with the [wrapping element from the template](../email-code/template#wrapping-element) if you like but if you're using multiple containers then the wrapping element should wrap all of the containers.
...
Generally most styling can be added to the `<div>` and it will work for both the MSO and HTML version.  But you may find issue with a few things like `padding` here there are a couple of options.  
...
The other option is to set all the styles for HTML on the `<div>` and all the styles for mso on the `<td>`.  We already have code to hide the table from the HTML version so now we can also add code to hide the HTML version from mso.
{% highlight html %}
<!--[if !true]><!-->
  <div style="max-width:37.5em; margin:0 auto">
<!--<![endif]-->
...
For the most part you can simple add the style to the body element like `<body style="background:#000000">`.  Then add a wrapping div around the entire content of the email `<div style="background:#000000">` will pick up most of the rest of the email clients.
...
Here I've changed the width to `width="100%"` to fill the screen and we set our background on this table.  Then I add a `<td></td>` either side of our content, creating a table with 3 cells. The middle `<td>` now takes the width and background of our container `style="width:37.5em;background:#ffffff"`
